...
export class DataQueue {
    ...
    nextWordLikeArray() {
        const item = this.elements[this.head];
        this.head += 1;
        const letters = item.word.split("").map((letter, i) => ({ letter, key: Math.random(), index: i }));
        return {
            ...item,
            letters,
    ...
    nextFour() {
        ...
        while (excludedElements.length !== 5) {
            elements = elements.filter(elem => elem.id !== excludedElements[excludedElements.length - 1].id);
...
export function reduceData(data: StatiscticsType, type: Exclude<ResultType, ResultType.sprint>): ResultPartType {
    return {
        ...
        accuracy: data[type].length
            ...
            : undefined,
        learned:
            type !== ResultType.puzzles && data[type].length ? getSum(data[type].map(item => item.learned)) : undefined,
        ...
        encountered:
            type !== ResultType.puzzles && data[type].length
                ...
                : undefined,
        time:
            (type === ResultType.puzzles || type === ResultType.constructor) && data[type].length
...
export function refineData(preData: StatiscticsType): RefinedResultsType {
    ...
    total.accuracy &&= +(total.accuracy / refinedData.filter(item => item.accuracy !== undefined).length).toFixed(3);
